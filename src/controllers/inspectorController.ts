...
export const getDashboard = async (req: Request, res: Response) => {
  try {
    ...
    /** Xe vào hàng chờ inspector: tin seller pending HOẶC đã admin duyệt sơ bộ, chưa xong kiểm định */
    const queueStatus = or(eq(bikes.status, 'pending'), eq(bikes.status, 'approved'));
...
export const getPendingBikes = async (req: Request, res: Response) => {
  try {
    ...
    // Tin mới seller (pending) + trường hợp cũ (approved trước khi kiểm định); chưa hoàn tất inspection
    const conditions = [
      or(eq(bikes.status, 'pending'), eq(bikes.status, 'approved')),
...
export const updateInspection = async (req: Request, res: Response) => {
  try {
    ...
    const proofFiles = (req.files as InspectionProofFiles | undefined)?.inspectionImages;
